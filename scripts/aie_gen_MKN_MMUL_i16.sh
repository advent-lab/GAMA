...
# Original folder
dir="aie/"
orig_folder=$dir"aie_i8_i16_32x32x32_api2x8x8"
echo $orig_folder
...
do
    ...
    new_folder=$dir"aie_i8_i16_${workload_m[i]}x${workload_k[i]}x${workload_n[i]}_api${mmul_m[j]}x${mmul_k[j]}x${mmul_n[j]}"
    echo $new_folder
    ...
    sed -i "s/const int L0_h1 = 32;/const int L0_h1 = ${h1};/g" "${new_folder}/kernels/para.h"
    sed -i "s/const int L0_w1 = 32;/const int L0_w1 = ${w1};/g" "${new_folder}/kernels/para.h"
    sed -i "s/const int L0_w2 = 32;/const int L0_w2 = ${w2};/g" "${new_folder}/kernels/para.h"

    # Replace the values in the file
    sed -i "s/const int M = 2;/const int M = ${mmul_h1};/g" "${new_folder}/kernels/mm_kernel0.cc"
